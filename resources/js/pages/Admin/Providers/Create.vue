...
const form = useForm({
    ...
    request_headers: [] as { key: string; value: string }[],
    request_body_template: [] as { key: string; value: string }[],
    response_mapping: [] as { key: string; value: string }[],
    timeout: 30,
    priority: 1,
    is_active: true,
    environment: 'live',
...
// Reset auth_config when auth_type changes
...
<template>
    <Head :title="`Add Provider - ${service.name}`" />
    <AdminLayout :user="$page.props.auth.user">
        <div class="mb-6">
            ...
            <h1 class="text-h4 font-weight-bold mb-1">Add Provider</h1>
            <p class="text-body-2 text-grey">Configure a new API provider for {{ service.name }}</p>
        </div>
        ...
        <v-form @submit.prevent="submit">
                ...
                <v-col cols="12" md="6">
                    <v-card class="mb-4">
                        ...
                        <v-card-text>
                            ...
                            <v-alert v-if="form.auth_type === 'none'" type="info" variant="tonal" density="compact">No authentication will be used for this provider.</v-alert>
                        </v-card-text>
                    </v-card>
                </v-col>
            </v-row>
                ...
                <v-col cols="12" md="6">
                    <v-card class="mb-4">
                        ...
                        <v-card-text>
                            ...
                            <v-alert v-if="!form.request_headers.length" type="info" variant="tonal" density="compact">
            ...
            <v-row>
                ...
                <v-col cols="12">
                    <v-card class="mb-4">
                        ...
                        <v-card-text>
                            <p class="text-body-2 text-grey mb-4">Map the API response fields to standard field names. Use dot notation for nested values (e.g., data.firstName).</p>
                            ...
                            <v-alert v-if="!form.response_mapping.length" type="info" variant="tonal" density="compact">
                                Define how to extract data from the API response.
                            </v-alert>
                        </v-card-text>
                    </v-card>
                </v-col>
            ...
            <div class="d-flex justify-end ga-2">
                ...
                <v-btn color="primary" type="submit" :loading="form.processing" prepend-icon="mdi-content-save">Create Provider</v-btn>
            </div>
        </v-form>
    </AdminLayout>
</template>
