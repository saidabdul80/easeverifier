...
const activeWalletBalance = computed(() => {
    ...
    return (
        props.branches?.find((branch) => branch.id === chargeBranchId.value)
            ?.wallet_balance || 0
    );
});
...
const optionsForField = (field: any) =>
    normalizeOptions(fieldOptions.value[field.name] || field.options);
...
const resultFormError = computed(
    () =>
        resultSubmitError.value ||
        (resultForm.errors as Record<string, string>).result,
);

const fieldError = (field: any) =>
    (resultForm.errors as Record<string, string>)[field.name];

const necoResultNotice = computed(() => {
    const board = String(
        props.resultBoard || props.service.slug || '',
    ).toLowerCase();
    ...
    if (['neco-everify', 'neco_everify', 'necoeverify'].includes(board)) {
        return {
            title: 'NECO e-Verify',
            message:
                'Use this only for students who bought or already have a token from NECO e-Verify.',
        };
    }
    ...
    if (props.isResultBoard) {
        ...
        resultForm
            ...
            .post(`/customer/verify/${props.service.id}`, {
                ...
                onError: (errors) => {
                    resultSubmitError.value = (errors.result ||
                        Object.values(errors)[0] ||
                        'Result verification failed. Please try again.') as string;
...
watch(
    () =>
        resultFields.value.map((field) =>
            field.depends_on ? resultForm[field.depends_on] : null,
        ),
    async () => {
                ...
                <v-card>
                    ...
                    <v-card-text>
                        ...
                        >
                            ...
                            Your selected wallet balance ({{
                                formatCurrency(activeWalletBalance)
                            }}) is less than the verification cost ({{
                        ...
                        <v-form @submit.prevent="submit">
                            <template v-if="isResultBoard">
                                ...
                                <v-alert
                                    v-if="necoResultNotice"
                                    type="info"
                                    variant="tonal"
                                    class="mb-4"
                                >
                                    <v-alert-title>{{
                                        necoResultNotice.title
                                    }}</v-alert-title>
                                    {{ necoResultNotice.message }}
                                </v-alert>
                        ...
                        <v-list density="compact">
                            ...
                            <v-list-item>
                                <v-list-item-title class="text-caption"
                                    >Your Balance</v-list-item-title
                                >
                            ...
                            <v-list-item>
                                <v-list-item-title class="text-caption"
                                    >Response Time</v-list-item-title
                                >
                                <v-list-item-subtitle
                                    >~2-5 seconds</v-list-item-subtitle
                                >
                            </v-list-item>
                        </v-list>
                    </v-card-text>
                ...
                <v-card class="mt-4">
                    ...
                    <v-card-text>
                        <ul class="text-body-2 text-grey pl-4">
                            <li class="mb-2">
                                Ensure the number is correct before verifying
                            </li>
                            <li class="mb-2">
